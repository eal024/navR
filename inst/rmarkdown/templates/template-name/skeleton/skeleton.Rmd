...
tabell3_2 <- bud$lagTabellMndUtviklingMottakere()
...
tabell2_2 <- tabell2 %>%
  select(-kategori) %>% 
  mutate_at( vars(regnskap, endring_regnskap  , regnskap_fast, endring_regnskap_f) ,function(x) format(x, digits = 0, big.mark = " ")) %>% 
  mutate( pris_snitt = format(pris_snitt, digits = 0, big.mark = " ")) %>% 
  mutate_at( vars(regnskap_vekst,regnskap_fast_vekst ) ,function(x) format(x*100, digits = 1)) %>%
  rename( 
...
tabell2_2 %>% 
  mutate_all(linebreak) %>%
  knitr::kable( format = "latex" , booktabs = T, escape = T, col.names = linebreak( names(tabell2_2) ) ) %>%  
...
tabell3_2 %>%
   mutate_all(linebreak) %>%
   kable(format = "latex", booktabs = T, escape = F,
      col.names = linebreak( names(tabell3_2) ), align = "c" ) %>% 
    kable_styling( latex_options = c("HOLD_position","striped", "scale_down"), stripe_index =  c(1,3,5,7,9,11) )  
