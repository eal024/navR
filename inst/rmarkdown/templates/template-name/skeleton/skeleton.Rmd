...
Dette er `r  round( endring_fra_forrige, digits = 1) %>% format( big.mark = " ")` mill. kroner `r endring_tekst` enn forrige anslag og  XXXX vedtatt budsjett. For 2021 tilrås et anslag på `r round( nytt_neste_ar$giSumAnslag()/10^6 , digits = 0) %>% format( big.mark = " " )` mill. kroner. 
...
knitr::kable( format = "latex", booktabs = T, escape = F,
              tabell2 %>% mutate_if( is.numeric, function(x){ ifelse(abs(x) > 1, round( x, digits = 0), round(x, digits = 2)) }) %>% 
                  rename( Endring = endring_regnskap,
                          ...
                          `Mill. kr i 2021-G` = regnskap_fast,
                          `Endring regnskap i 2021-G`  = endring_regnskap_f,
